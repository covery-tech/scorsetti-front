          ...
          <tbody>
            {pas.map((e,i) => (
                ...
                <tr key={i}>
                  ...
                  <td>{e?.id_user}</td>
                  <td><Link to={`user/${e?.id_user}`} style={{color:"#0d6efd",cursor:"pointer"}} >Ver usuario</Link></td>
                  <td className="content-center">
                    <div className="mx-auto">
                      {
                        e?.status_pas ? <button className="btn btn-success" onClick={()=>onChangeEnable(e?.status_pas,e?.id_user)}>habilitado</button> : <button className="btn btn-warning" onClick={()=>onChangeEnable(e?.status_pas,e?.id_user)}>deshabilitado</button>
                      }
                    </div>
                  </td>
                </tr>
