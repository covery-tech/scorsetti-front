export default function Validate(values) {
  ...
  if (!values.house_type) {
    errors.house_type = 'Ingrese el tipo de vivienda';
  }
  if (!values.house_use) {
    errors.house_use = 'Ingrese el uso de la vivienda';
  }
  if (!values.risk_location) {
    errors.risk_location = 'Ingrese la dirección del riesgo';
  }
  if (!values.meters) {
    errors.meters = 'Ingrese los mts2 cubiértos';
  }
  if (!values.material_type) {
    errors.material_type = 'Ingrese el material de construcción';
  }
  if (!values.roof_type) {
    errors.roof_type = 'Ingrese el tipo de tejado';
  }
