...
export const TableNotisAdmin = ({notis,setPage,loading,showShowMore,loadingNextPage,setLoading,setLoadingNextPage,setNotis,page,setShowShowMore}) => {
    const {jwt} = useUser() 
    //(notis)
    const getNotis = ()=>{
        const config = {
    ...
    const deleteNoti = (id)=>{
        const config = {
            method:"put",
            baseURL: `${process.env.REACT_APP_URI_API}/product/deleteNotificationAdmin/${id}`,
            headers:{token:jwt}
        }
        axios(config).then(e=>{
            if(e.data){
        ...
        loading 
            ...
            <tbody>
                {notis.map((e) => (
                    ...
                    <td>{e?.email}</td>
                    <td>{e?.description}</td>
                    <td><Link to={`user/${e?.idPas}`} style={{color:"#0d6efd",cursor:"pointer"}} >Ver Usuario</Link></td>
                    <td><strong style={{color:"#dc3545",cursor:"pointer"}} onClick={()=>deleteNoti(e?.id)} >Eliminar</strong></td>
                    </tr>
                ))}
            </tbody>
            ) : (
