...
export const DataTablePas = ({e,handleUpdateState,sendNotification}) => {
  ...
  return <><tr>
            ...
            <td>
                {
                    ...
                    <>  
                        ...
                        <strong>click en el boton si quiere poner este producto en pendiente por deshabilitar</strong>
                    </>
                : e.status === "pendiente deshabilitado" ?
                <><button className='btn btn-warning' onClick={()=>handleUpdateState(e.name,"habilitado")}>{e.status}</button>
                    <strong>click para cambiar cancelar esta solicitud</strong></>
                : e.status === "pendiente habilitado" ? 
                <><button className='btn btn-warning' onClick={()=>handleUpdateState(e.name,"deshabilitado")}>{e.status}</button>
                <strong>click para cambiar cancelar esta solicitud</strong></>
                ...
                <strong>click en el boton si quiere poner este producto en pendiente por habilitar</strong></>
                }
            </td>
        </tr>
        { actionSureDisable && <ModalPortal onClose={setActionSureDisable}>
            <div>
                <h4>esta seguro que quiere enviar una solicitud deshabilitar el producto {e.name}?</h4>
                <button onClick={()=>{
                            handleUpdateState(e.name,"pendiente deshabilitado")
                            sendNotification(`${user.name},${user.last_name} desea acutalizar el estado de su producto ${e.name} a deshabilitado`)
        ...
        { actionSureEnable && <ModalPortal onClose={setActionSureEnable}>
            <div>
                <h4>esta seguro que quiere enviar una solicitud habilitar el producto {e.name}?</h4>
                <button onClick={()=>{
                            handleUpdateState(e.name,"pendiente habilitado")
                            sendNotification(`${user.name},${user.last_name} desea acutalizar el estado de su producto ${e.name} a habilitado`)
